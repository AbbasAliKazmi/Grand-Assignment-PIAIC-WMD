...
//3
let sen = 'Abbas is a very good boy';
...
//37
function make_shirt_v2(size, message) {
    console.log(`The shirt size is ${size} and the message printed on it is "${message}".`);
}
...
console.log(album4);
//41
let magicians = ['Kallu', 'Taklu', 'Shapar'];
...
function show_magicianz(magicianz) {
    for (let magician of magicianz) {
        console.log(magician);
    }
}
let magicianz = ['Kallu Jadugar', 'Taklu Jadugar', 'Shapar Jadugar'];
make_great(magicianz);
show_magicianz(magicianz);
//43
const jadugar = ['Kallu Jadugar', 'Taklu Jadugar', 'Shapar Jadugar'];
function make_greater(names) {
    const greatNames = names.map(name => `Great ${name}`);
    return greatNames;
}
function show_magicianss(names) {
    names.forEach(name => console.log(name));
}
const greatMagicians = make_greater([...jadugar]);
